export const getLoading = (state) => state.utilsStore.loading
export const getLoadingContent = (state) => state.utilsStore.loadingContent

export const getError = (state) => state.utilsStore.error
export const getLanguagePicker = (state) => state.utilsStore.languagePicker
export const getLanguage = (state) => state.utilsStore.language
export const getLogsQueue = (state) => state.utilsStore.logsQueue
export const getSocketConnection = (state) => state.utilsStore.socketConnection
export const getNotifications = (state) => state.utilsStore.notifications
export const getModalOpened = (state) => state.utilsStore.modalOpened
export const getModalContent = (state) => state.utilsStore.modalContent
export const getSelectedChallenge = (state) => state.utilsStore.selectedChallenge
export const getSidePanel = (state) => state.utilsStore.sidePanel
export const getCode = (state) => state.utilsStore.code
export const getDifficulty = (state) => state.utilsStore.difficulty
